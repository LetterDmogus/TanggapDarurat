                ...
                {(type === 'line' || type === 'area') &&
                    points.map((point) => (
                        <g key={`dot-${point.label}-${point.x}`}>
                            ...
                            <circle cx={point.x} cy={point.y} r="10" fill="transparent" onMouseEnter={() => setActivePoint(point)} />
                        </g>
                    ))}
...
export default function Dashboard({ analytics }) {
    ...
    return (
        <ManagerLayout
            header={<h2 className="text-xl font-bold tracking-tight text-surface-900">Manager Dashboard</h2>}
        >
                        ...
                        <div className="flex items-start gap-2">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-blue-700">
                                    Ada <span className="font-bold">{analytics.summary.triage_reports}</span> laporan kategori Lainnya.
                                </p>
                            </div>
                        </div>
                        <Link href={route('manager.reports.index', { status: 'triage' })} className="btn-primary text-xs">
                            Buka Laporan
                        </Link>
                    </div>
                </div>
                ...
                <div className="rounded-xl border border-amber-200 bg-amber-50 p-4">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                        <div className="flex items-start gap-2">
                            ...
                            <div>
                                <p className="text-sm font-semibold text-amber-800">Menunggu Validasi Admin</p>
                                <p className="text-sm text-amber-700">
                                    Ada <span className="font-bold">{analytics.summary.waiting_validation_reports}</span> laporan menunggu validasi.
                                </p>
                            </div>
                        </div>
                        <Link href={route('manager.reports.index', { status: 'resolved_waiting_validation' })} className="btn-primary text-xs">
                            Buka Laporan
                        </Link>
                    </div>
                </div>
