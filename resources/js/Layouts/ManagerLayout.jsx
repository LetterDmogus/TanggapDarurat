...
export default function ManagerLayout({ header, children }) {
    ...
    const navigation = [
        { name: 'Dashboard', href: route('manager.dashboard'), icon: LayoutDashboard, active: route().current('manager.dashboard') },
        { name: 'Laporan', href: route('manager.reports.index'), icon: ClipboardList, active: route().current('manager.reports.*') },
    ];

    return (
        <div className="min-h-screen bg-surface-100 flex font-sans">
            <aside
                className={`fixed inset-y-0 left-0 z-50 w-72 bg-gradient-sidebar text-white transition-transform duration-300 lg:static lg:translate-x-0 ${
            ...
            >
                <div className="h-full flex flex-col p-6">
                    ...
                    <nav className="flex-1 space-y-2">
                        ...
                        {navigation.map((item) => (
                            <SidebarLink key={item.name} href={item.href} active={item.active} icon={item.icon}>
                                {item.name}
                            </SidebarLink>
                        ))}
                    </nav>
            ...
            <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
                ...
                <header className="h-16 bg-white border-b border-surface-200 flex items-center justify-between px-6 sticky top-0 z-30">
                    <div className="flex items-center gap-4">
                        <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="btn-icon lg:hidden">
                            <Menu className="w-6 h-6" />
                        </button>
                        {header && <div className="animate-fadeIn">{header}</div>}
                    </div>
